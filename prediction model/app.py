...
app = Flask(__name__)
model = pickle.load(open('LR_c.pkl', 'rb'))# loads ML model
...
def predict():
    ...
    if output == 1:
        output  = "Customer may churn."
    else:
        output = "Customer may not churn"

    return render_template('index.html', prediction_text='Customer Prediction: {}'.format(output))# displays the prediction inside the 'b{{ prediction_text }}b' that we've seen in 'index.html'
